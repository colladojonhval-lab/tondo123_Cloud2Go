...
void AirlineBookingManager::loadData() {
    ...
    if (!file) {
        ...
        flights.push_back(Flight(103, "Singapore, singapore", 2000));
        ...
        flights.push_back(Flight(123, "New Delhi, India", 12000));
    ...
    for (int i = 0; i < flights.size(); i++) {
        if (flights[i].getFlightNumber() == fNum) {
            ...
            if (sNum >= 1 && sNum <= 5 && !flights[i].isSeatTaken(sNum)) {
                ...
                history.push_back({ name, fNum, sNum });
...
void AirlineBookingManager::updatePassengerInfo() {
    ...
    for (int i = 0; i < history.size(); i++) {
        if (history[i].pName == target) {
            cout << "New Name: "; getline(cin, history[i].pName);
            cout << "Success!\n";
            system("pause");
            return;
        }
